...
Two independent groupings both live in this plugin and both ship from it —
consolidating hook sprawl does not mean moving these elsewhere (see
`meta/superpowers/plans/2026-09-14-hooks-sprawl-evaluation.md`):

- **Family A — product/continuity.** Everything below in this section:
  SessionStart, the commit nudge, action-keyed LEARNINGS retrieval, and the
  seven content gates. Runs as one multiplexed process at commit time
...
- **Family B — operator safety.** Blocking gates unrelated to commits:
  ...
  scoped to `Bash(git commit *)` — they have nothing to do with commits.
...
- **Occurrence gate** (`occurrence-gate`, staged `LEARNINGS.md` only) blocks a newly added 2nd-or-later occurrence of a mistake-class (`Occurrence count: N of M`, N ≥ 2) unless the staged document names an end-state `Invariant:` line — the thing that, enforced at the reconciler/entry gate, makes the whole class impossible rather than patching one more trigger. A first occurrence (or no count) never fires. Override with `Occurrence-gate: N/A — <reason>`.
- **Evidence gate** (`evidence-gate`, staged spec/plan files only) blocks newly added smoke-design prose that tears down a test subject without saying the failure diagnostic is captured first, or adds a poll/wait without watching both success and failure signals. Override with `Evidence-gate: N/A — <reason>`.
- **Flaky gate** (`flaky-gate`, commit messages and staged `LEARNINGS.md`) blocks a commit message or newly added LEARNINGS prose that calls a failure "flaky" / "transient" / "CDN blip" without a `Mechanism:` naming its deterministic cause (a race, shared state, an environment dependency). The commit-message check still scans the whole message. Override with `Flaky-gate: N/A — <reason>`.
- **Backend-parity gate** (`backend-parity-gate`, staged plan files only) blocks newly added multi-backend smoke coverage that names only one concrete backend instead of a second for parity coverage. Single-backend projects are unaffected unless the added plan text uses "backend" or "backends." Override with `Backend-parity: N/A — <reason>`.
- **Derived-value gate** (`derived-value-gate`, staged `commands/*.md` only) blocks newly added command prose that tells a model to hand-compute a duration, tally/vote a count by eye, eyeball-compare a claimed value against an actual one, or print fixed reference text as if it were an instruction — the reconciler for the determinism program. Override with `Derived-value-gate: N/A — <reason>`.
