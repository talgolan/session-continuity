...
This skill ships a specific pattern: four in-repo Markdown files (`.session-continuity/SESSION_PRIMER.md`, `.session-continuity/PROJECT_CONTEXT.md`, `.session-continuity/ROADMAP.md`, and `.session-continuity/LEARNINGS.md`) plus GitHub Issues labeled `backlog` for the work queue, with slash commands and hooks that make the habit of using them cheap. That's the whole scope.
...
- A fifth in-repo doc (e.g. `.session-continuity/DECISIONS.md`, `.session-continuity/DECISIONS_LOG.md`). The four-file split plus GitHub Issues is load-bearing; see the README.
...
The marketplace catalog that lets users `/plugin marketplace add` this plugin no longer lives in this repo — it moved to the separate `talgolan/claude-plugins` repo (see the README's "Install" section). `.session-continuity/` here is intentionally limited to the four files the plugin ships — primer, PROJECT_CONTEXT, ROADMAP, and LEARNINGS — so it mirrors what users see in their own projects. Repo-specific dev artifacts (design specs, implementation plans, submission paperwork) live under `meta/` so they don't pollute the plugin's public surface.
